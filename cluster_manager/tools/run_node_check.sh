...
# =============================================================================
# Node Check Launch Script
# 节点检测启动脚本
# =============================================================================
#
# 使用方式：
#   ./run_node_check.sh <clushnode_file> [nodenum] [options]
#
# 示例：
#   ./run_node_check.sh ./clushnode 4
#   ./run_node_check.sh -c ./clushnode -n 4 -t 100
#
# 参数：
#   $1 - clushnode 文件路径（必填）
#   $2 - 每组节点数（可选，默认4）
...
# node_check 脚本目录
NODE_CHECK_DIR="${PROJECT_ROOT}/cluster_manager/node_check"

# 默认值
CLUSHNODE=""          # 必填：clush节点列表文件
OUTPUT="${SCRIPT_DIR}/node_check_results"
...
average_tflops=100    # 可选：TFLOPs阈值（默认100）
...
usage() {
    ...
    echo "  -g, --healthy <file>     可选：指定健康节点保存路径（默认：./node_checklog/host_check_pass）"
    echo "  -f, --fault <file>       可选：指定异常节点保存路径（默认：./node_checklog/host_check_error）"
...
# 检查是否提供了 clushnode 参数
if [[ -z "$CLUSHNODE" ]]; then
    echo -e "${RED}Error: 缺少 clushnode 参数${NC}"
    echo ""
    echo "使用方式："
    echo "  $0 <clushnode_file> [nodenum]"
    echo ""
    echo "示例："
    echo "  $0 ./clushnode 4"
    echo "  $0 -c ./clushnode -n 4"
    exit 1
fi

# 检查 clushnode 文件是否存在
if [[ ! -f "$CLUSHNODE" ]]; then
    echo -e "${RED}Error: clushnode 文件不存在: $CLUSHNODE${NC}"
    exit 1
fi

# 检查 check_nodes.sh 是否存在
if [[ ! -f "${NODE_CHECK_DIR}/check_nodes.sh" ]]; then
    echo -e "${RED}Error: check_nodes.sh 不存在: ${NODE_CHECK_DIR}/check_nodes.sh${NC}"
    exit 1
fi
...
do
    ...
    ${NODE_CHECK_DIR}/check_nodes.sh ${SCRIPT_DIR}/hostslice/horizontal_host_${group_index} \
...
for ((v=0; v<vertical_groups_num; v++)); do
    ...
    # Run test
    ${NODE_CHECK_DIR}/check_nodes.sh "${vertical_hostfile}" \
...
for log in ${SCRIPT_DIR}/node_checklog/vertical_V*.log; do
    ...
    host_file="${SCRIPT_DIR}/hostslice/vertical_host_${group_index}"
