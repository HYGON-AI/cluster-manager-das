...
def filter_cluster_manager_logs(line: str, log_stack, cluster_last_time_wrapper):
    ...
    filter_ft_logs(line, log_stack, cluster_last_time_wrapper)
...
"""
过滤以下信息：
    1、FT: initialized. Timeouts=SectionTimeouts(out_of_section=300.00, sections=[setup=600.00, step=180.00, checkpointing=420.00, ], calculated_sections=set(), is_out_of_section_calculated=False)
    2、2026-02-03 11:22:31,089 [WARNING] [a10r1n13] [workload:2(2) infra:?(?)] rank_monitor_server.py:467 Was out of section for 303.46 seconds. Timeout is 300.00 seconds.
    3、2026-02-03 11:22:31,279 [INFO] [a10r1n13] [workload:0(0) infra:?(?)] rank_monitor_server.py:192 Started periodic restart check.
    4、2026-02-03 11:22:31,277 [INFO] [a10r1n13] [workload:?(?) infra:?(?)] launcher.py:519 2/2 restart attempts left; restarting worker group...
    5、2026-02-03 11:24:08,386 [INFO] [a10r1n14] [workload:?(?) infra:?(?)] launcher.py:639 Stopping workers... Timeout = 15 sec.
    4、2026-02-03 11:24:46,060 [INFO] [a10r1n14] [workload:?(?) infra:?(?)] launcher.py:528 0/2 restart attempts left. Exiting...
...
class FTEventType:
    OUT_OF_SECTION_TIMEOUT = "OUT_OF_SECTION_TIMEOUT"
    RESTARTING_WORKERS = "RESTARTING_WORKERS"
    EXIT_NO_RESTART_LEFT = "EXIT_NO_RESTART_LEFT"
    
_recent_ft_events_cache = {}
_FT_EVENT_SUPPRESS_SECONDS = 10  # 10 秒内不重复打印同类事件

def filter_ft_logs(log_line: str, log_stack, cluster_last_time_wrapper):
    global _recent_ft_events_cache

    timestamp_pattern = r'(\d{4}-\d{2}-\d{2}\s\d{2}:\d{2}:\d{2})'
    matches = list(re.finditer(timestamp_pattern, log_line))

    if matches:
        for i, match in enumerate(matches):
            start_pos = match.start()
            end_pos = matches[i + 1].start() if i + 1 < len(matches) else len(log_line)
            single_log_entry = log_line[start_pos:end_pos].strip()

            _process_single_log_entry(single_log_entry, log_stack, cluster_last_time_wrapper)
            # return "\n".join(results)
            return True

def _process_single_log_entry(log_line: str, log_stack, cluster_last_time_wrapper):
    global last_recorded_cluster_time
    global _recent_ft_events_cache
    ...
    FT_EVENT_RULES = [
        {
            "type": FTEventType.OUT_OF_SECTION_TIMEOUT,
            "pattern": re.compile(
                r'rank_monitor_server\.py:\d+.*out of section for (?P<elapsed>\d+(\.\d+)?) seconds\. Timeout is (?P<limit>\d+(\.\d+)?) seconds'
            ),
            "parser": lambda m: {
                "event": FTEventType.OUT_OF_SECTION_TIMEOUT,
                "message": f"时限为{m.group('limit')}s，当前已超时",
            },
        },
        {
            "type": FTEventType.RESTARTING_WORKERS,
            "pattern": re.compile(
                r'(?P<cur>\d+)/(?P<total>\d+)\s+restart attempts left; restarting worker group\.{3}'
            ),
            "parser": lambda m: {
                "event": FTEventType.RESTARTING_WORKERS,
                "message": f"ft_launcher正在尝试重启，当前重启次数为{m.group('cur')}/{m.group('total')}",
                "current_attempt": int(m.group("cur")),
            },
        },
        {
            "type": FTEventType.EXIT_NO_RESTART_LEFT,
            "pattern": re.compile(
                r'0/(?P<total>\d+)\s+restart attempts left\. Exiting\.{3}'
            ),
            "parser": lambda m: {
                "event": FTEventType.EXIT_NO_RESTART_LEFT,
                "message": "已无重启次数，ft_launcher退出，后续使用cluster重启",
            },
        },
    ]


    log_time = pytz.timezone('Asia/Shanghai').localize(
    ...
    for rule in FT_EVENT_RULES:
        match = rule["pattern"].search(log_line)
        if not match:
            continue

        event_info = rule["parser"](match)
        event_type = event_info["event"]
        event_msg = event_info["message"]

        # === 生成 cache_key（与你原逻辑保持一致）===
        if event_type == FTEventType.OUT_OF_SECTION_TIMEOUT:
            cache_key = event_type
        elif event_type == FTEventType.RESTARTING_WORKERS:
            cache_key = f"{event_type}_{event_info.get('current_attempt', 0)}"
        else:
            cache_key = event_type

        # === 核心抑制逻辑 ===
        last_print_time = _recent_ft_events_cache.get(cache_key)
        if last_print_time:
            if (log_time - last_print_time).total_seconds() < _FT_EVENT_SUPPRESS_SECONDS:
                return   # 10s 内直接丢弃

        # 记录本次打印时间
        _recent_ft_events_cache[cache_key] = log_time
...
def send_message_from_log(model_name, training_log, is_test=False, abnormal_threshold=ABNORMAL_THRESHOLD):
    ...
    if log_stack:
        ...
        if FTEventType.RESTARTING_WORKERS in reversed_log_content or FTEventType.OUT_OF_SECTION_TIMEOUT in reversed_log_content:
            hang_msg = (
                f"{test_info}【警告】{model_name}模型训练容错介入！\n"
                f"当前时间: {time_print}\n"
