...
# Modified by Hygon Information Technology Co., Ltd. for HCU support.
#
...
class FaultToleranceProfiler:
    ...
    def __init__(self):
        ...
        if not TrainingTelemetryProvider.instance().one_logger_ready:
            ...
            if os.environ.get("SLURM_JOB_ID"):
                # Slurm 环境
                session_tag = os.environ.get("SLURM_JOB_ID", "local_run")
                custom_metadata = {
                    "launcher": "slurm",
                    "local_host": local_hostname,
                }
            elif os.environ.get("OMPI_COMM_WORLD_SIZE"):
                # MPI 环境 (OpenMPI)
                session_tag = self._get_mpi_session_tag()
                custom_metadata = {
                    "launcher": "mpirun",
                ...
                # 其他环境
                session_tag = f"local_{int(time.time())}"
                custom_metadata = {"launcher": "local"}
            
            # print(f'session_tag:{session_tag}')
            # print(f'custom_metadata:{custom_metadata}')
            
            # 确保 custom_metadata 中没有 None 值
            if custom_metadata:
                filtered_metadata = {}
                for key, value in custom_metadata.items():
                    if value is not None:
                        filtered_metadata[key] = value
                custom_metadata = filtered_metadata if filtered_metadata else None
    ...
    def _get_mpi_session_tag(self):
        """为 MPI 环境生成会话标签"""
        # 尝试使用 MPI 作业 ID 或生成唯一标识
        mpi_job_id = os.environ.get("MPI_JOB_ID") 
        if mpi_job_id:
            return mpi_job_id
        ...
        return f"mpi_{hostname}_{timestamp}"
